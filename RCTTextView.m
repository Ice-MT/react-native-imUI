...
- (void)updateFrames
{
   ...
   *  -----------------------------------------------
   */
  adjustedTextContainerInset.top = self.forIM?0:5;
  /**
   *  -----------------------------------------------
   */
...
- (void)updateContentSize
{
  ...
  if (size.height>_scrollView.frame.size.height) {
    [_scrollView setContentOffset:CGPointMake(0, size.height-_scrollView.frame.size.height) animated:NO];
  }
  /**
   *  -----------------------------------------------
   */
...
- (void)setDeleteBackward:(BOOL)deleteBackward
{
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (deleteBackward) {
      ...
      _onSelectionChange(@{@"actionType":@"del"});
    }
  });
}
/**
...
- (BOOL)textView:(RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
   ...
   *  -----------------------------------------------
   */
  if (textView == _textView&&self.forIM) {
    if ([text isEqualToString:@"\n"]){
      _onSelectionChange(@{@"actionType":@"send"});
      return NO;
    }
    return YES;
  }
